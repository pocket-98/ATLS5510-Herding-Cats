...
public class CenterSpawnManager : MonoBehaviour
{
    ...
    private void PositionScoreboard(MRUKAnchor table)
    {
        ...
        Vector3 scoreboardPositionInitial = new Vector3(center.x, center.y + ((tableHeightFromCenter - center.y) * 2), center.z);
        ...
        Vector3 scoreboardPosition = new Vector3(scoreboardPositionInitial.x + (largestDistanceIndex == 'X' ? (playerPosition.x > scoreboardPositionInitial.x ? -0.5f : 0.5f) : 0), scoreboardPositionInitial.y, scoreboardPositionInitial.z + (largestDistanceIndex == 'Z' ? (playerPosition.z > scoreboardPositionInitial.z ? -0.5f : 0.5f) : 0));
        ...
        // Rotate the scoreboard to face the player on only the x or y axis depending on how far the player is on each
        scoreboardCanvas.transform.LookAt(new Vector3(largestDistanceIndex == 'X' ? playerPosition.x : scoreboardPositionInitial.x, scoreboardPositionInitial.y, largestDistanceIndex == 'Z' ? playerPosition.z : scoreboardPositionInitial.z));
